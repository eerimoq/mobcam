...
in `crates/virtualcam` (`convert.rs`, `audio.rs`, `alsa.rs`) and `crates/core`
(`ffmpeg/version.rs`); `mobcam-obs-plugin` sets `test = false` because it is a bare `cdylib` against
libobs.

`tests/` is the system test suite, in Python: it streams from an iPhone or iPad running Moblin,
records what comes out and asserts on the recording. Read `tests/README.md` before touching it — the
tests need a device on a USB cable, a `tests/config.toml` copied from `tests/config.example.toml`
...
`virtualcam/build.rs` runs no bindgen. It probes `pkg-config` for `libpulse-simple` and `alsa` and
emits `cfg(pulse)` / `cfg(alsa)` — these are build-script cfgs, not Cargo features, so
`#[cfg(pulse)]` code is only compiled where the library was present at build time. A machine with
...
```
usbmux::Stream  →  Session::attempt()  →  protocol::unpack_*  →  Decoder  →  Sink
```
...
- `core/protocol.rs` is the Moblin wire format: a 5 byte header (kind, big-endian length) then hello,
  ...
  the caller's buffer.
- `core/session.rs` owns the message loop. `Session::attempt()` connects, builds the consumer's
  handler and runs a `Connection` until the device goes away. It defines `Handler: Sink` — the one
  trait a consumer implements: `hello()` for the device name, plus `Sink::video`/`Sink::audio` for
  decoded frames.
...
- `core/clock.rs` turns the device's presentation timestamp into one on the host's monotonic clock,
  re-anchoring on any PTS jump over five seconds. Both products feed their frames through it.
- `core/panic.rs` is `panic::guard`, which catches an unwind at every boundary a Rust panic must not
  cross: each `extern "C"` entry point of the plugin, and the plugin's receive thread.
...
**OBS plugin.** `obs/` wraps libobs the same way `ffmpeg/` wraps FFmpeg: `Source`, `Data` and
`Properties`, plus `obs/media.rs`, which holds the `Frame`/`Audio` aliases and the mapping from
FFmpeg pixel formats, colorspaces, sample formats and channel counts to the libobs ones. `source.rs`
is the source type — a worker thread reconnects every second and an `Output` implements `Sink` and
pushes into libobs, stamped with `core::clock::Clock`. `devices.rs` remembers the name of every
device that has said hello and saves it into the source settings, so the property dropdown can list
a device that is not attached right now. Every `extern "C"` entry point goes through `panic::guard`
so a Rust panic never unwinds into C. UI strings are keys looked up via `obs::text()` against
...
while `VIDIOC_QBUF` carries a timestamp of the caller's choosing, which is the one `core/clock.rs`
hands out. The buffer index has to be a strict rotation, as the output `VIDIOC_DQBUF` of
