...
mod tests {
    ...
    #[test]
    fn interleaved_audio_is_one_plane_whatever_the_layout() {
        for speakers in [sys::SPEAKERS_MONO, sys::SPEAKERS_STEREO, sys::SPEAKERS_7POINT1] {
            assert_eq!(audio_planes(sys::AUDIO_FORMAT_16BIT, speakers), 1);
        }
    }

    #[test]
    fn planar_audio_is_one_plane_per_channel() {
        assert_eq!(audio_planes(sys::AUDIO_FORMAT_FLOAT_PLANAR, sys::SPEAKERS_STEREO), 2);
        assert_eq!(audio_planes(sys::AUDIO_FORMAT_16BIT_PLANAR, sys::SPEAKERS_5POINT1), 6);
    }

    #[test]
    fn the_channel_table_matches_the_obs_headers() {
        assert_eq!(audio_channels(sys::SPEAKERS_MONO), 1);
        assert_eq!(audio_channels(sys::SPEAKERS_STEREO), 2);
        assert_eq!(audio_channels(sys::SPEAKERS_2POINT1), 3);
        assert_eq!(audio_channels(sys::SPEAKERS_4POINT0), 4);
        assert_eq!(audio_channels(sys::SPEAKERS_4POINT1), 5);
        assert_eq!(audio_channels(sys::SPEAKERS_5POINT1), 6);
        assert_eq!(audio_channels(sys::SPEAKERS_7POINT1), 8);
        assert_eq!(audio_channels(sys::SPEAKERS_UNKNOWN), 0);
    }

    /// Every channel count the decoder maps has to round-trip, or audio would
    /// be handed to OBS with the wrong number of planes.
    #[test]
    fn the_speaker_and_channel_tables_agree() {
        for channels in [1, 2, 3, 4, 5, 6, 8] {
            let layout = speakers(channels).expect("a layout for {channels} channels");

            assert_eq!(audio_channels(layout), channels as usize);
        }
    }
}
