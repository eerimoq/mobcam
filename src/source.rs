...
mod tests {
    ...
    /// The two streams share one timeline, so audio that starts a little before
    /// the video that anchored it must not be placed before the origin.
    #[test]
    fn an_earlier_stream_moves_the_origin_back() {
        let mut clock = Clock::default();

        let video = clock.timestamp(1_000_000);
        let audio = clock.timestamp(999_000);

        assert!(audio < video, "the earlier timestamp stays earlier");
        assert_eq!(video - audio, 1_000_000, "and by the distance between them");
    }

    #[test]
    fn timestamps_keep_their_spacing() {
        let mut clock = Clock::default();

        let first = clock.timestamp(1_000_000);
        let second = clock.timestamp(1_033_000);

        assert_eq!(second - first, 33_000_000, "33 ms in nanoseconds");
    }

    #[test]
    fn a_large_jump_starts_a_new_timeline() {
        let mut clock = Clock::default();
        ...
        clock.timestamp(1_000_000 + PTS_DISCONTINUITY_US + 1);

        assert_ne!(clock.anchor_ns, anchor, "it anchored again");
        assert_eq!(clock.first_pts_us, 1_000_000 + PTS_DISCONTINUITY_US + 1);
    }

    #[test]
    fn the_name_cache_keeps_the_latest_name_per_device() {
        name_cache_set("serial-a", "First");
        name_cache_set("serial-a", "Second");
        name_cache_set("serial-b", "Other");

        assert_eq!(name_cache_get("serial-a").as_deref(), Some("Second"));
        assert_eq!(name_cache_get("serial-b").as_deref(), Some("Other"));
        assert_eq!(name_cache_get("serial-unknown"), None);
    }

    #[test]
    fn the_name_cache_ignores_empty_entries() {
        name_cache_set("", "Name");
        name_cache_set("serial-c", "");

        assert_eq!(name_cache_get(""), None);
        assert_eq!(name_cache_get("serial-c"), None);
    }
}
